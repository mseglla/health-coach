import { totalBurn, averageWeight } from './calculations.js';
...
export function getCoachDecision(state, day) {
  ...
  const weights = state.days.filter(item => item.weight);

  if (!day.weight) return { tone: 'neutral', label: 'SENSE DADES', title: 'Pesa’t i registra el dia.', caption: 'Comença pel pes' };
  if (day.intake == null || day.active == null) return { tone: 'warn', label: 'DIA INCOMPLET', title: 'Completa calories ingerides i actives.', caption: 'Falten dades' };

  if (weights.length >= 14) {
    const first = averageWeight(weights, 7, 7);
    const second = averageWeight(weights, 7, 0);
    if (first != null && second >= first - 0.1) {
